...
if (isset($user) && $user->runscript()) {
	...
	if(isset($_GET['frei']) && $_GET['frei'] == 0) {
		$return = query_db("SELECT * FROM `raum` WHERE frei = 0 ORDER BY `raum`.`nummer` ASC, `raum`.`tag` ASC, `raum`.`stunde` ASC");
	}else if(isset($_GET['frei']) && $_GET['frei'] == 1){
		$return = query_db("SELECT * FROM `raum` WHERE frei = 1 ORDER BY `raum`.`nummer` ASC, `raum`.`tag` ASC, `raum`.`stunde` ASC");
	}else{
		$return = query_db("SELECT * FROM `raum` WHERE 1 ORDER BY `raum`.`nummer` ASC, `raum`.`tag` ASC, `raum`.`stunde` ASC");
	}
	if($return !== false) {
		$result = $return->fetch();
		if (!$result) {
		...
		while ($result){
			echo "<tr><td>".$result['nummer']."</td><td>".get_name_of_tag($result['tag'])."</td><td>".$result['stunde']."</td><td>".($result['frei'] == 1 ? "frei" : "belegt durch Nachhilfeppar")."</td><td>
